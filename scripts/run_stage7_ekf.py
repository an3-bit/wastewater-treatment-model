"""
Extended Kalman Filter (EKF) Benchmark Execution Script (Stage 7).
...
5. Generates results/stage7/tables/ekf_benchmark.csv
...
def run_ekf_on_trajectory(
    ...
    """Execute EKF on a synthetic trajectory and return metrics and history."""
    ...
    # Determine initial state estimate
    ...
    if forced_clean_init_on_fouled:
        ...
        # Apply initial state perturbation
    ...
    for step_data in records:
        ...
        # Select noisy measurement based on noise level
...
def run_stage7_ekf_benchmark():
    print("=" * 80)
    print("STAGE 7: EXTENDED KALMAN FILTER (EKF) BENCHMARK SUITE")
    ...
    for strat in strategies:
        ...
        print(f"  Running EKF on {strat} (Model B, Standard Sensors)...")
        res = run_ekf_on_trajectory(traj, representation=StateRepresentation.AXIAL_6_ZONE)
        m = res["metrics"]
    ...
    for err in init_errors:
        print(f"  Running EKF Initialization Robustness: +{int(err*100)}% Error...")
        res = run_ekf_on_trajectory(traj_d, initial_error_fraction=err)
        m = res["metrics"]
        rows.append({
    ...
    print("  Running EKF on Unknown Partially Fouled Start (Assumed Clean Initial Condition)...")
    res_part = run_ekf_on_trajectory(traj_part, forced_clean_init_on_fouled=True)
    m = res_part["metrics"]
    rows.append({
        "test_category": "Initialization Robustness",
    ...
    for n_level in [NoiseLevel.LOW, NoiseLevel.NOMINAL, NoiseLevel.HIGH]:
        print(f"  Running EKF Noise Sensitivity: {n_level.value} Noise...")
        res = run_ekf_on_trajectory(traj_d, noise_level=n_level)
        m = res["metrics"]
        rows.append({
    ...
    for s_rep, name_str in [
        ...
        print(f"  Running EKF State Representation: {name_str}...")
        res = run_ekf_on_trajectory(traj_d, representation=s_rep)
        m = res["metrics"]
        rows.append({
    ...
    df.to_csv(tables_dir / "ekf_benchmark.csv", index=False)
    print(f"\n[OK] Saved EKF benchmark summary table to {tables_dir / 'ekf_benchmark.csv'}")


if __name__ == "__main__":
    run_stage7_ekf_benchmark()
